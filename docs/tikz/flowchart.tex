...
\begin{tikzpicture}[node distance=2cm, thick, scale=.5, every node/.style={transform shape},
  ...
  \coordinate (chain_c)    at (COORD\nrows_\ncols);
  \coordinate (future_c)   at ($ (chain_c) - (0,2.5) $);
  ...
  \node[align=left, thick] at ($ (client) - (-2.5,5.5) $) {\huge \textbf{``Client''}};

  % Event ntuples
  \foreach \i in {1,...,-1}{
    \ifnum\i=-1
    \node (entuple\i) [myredbox1] at ($ (entuple_c) + (\i/2,\i/2) $)
    {\huge \twolines{Events}{Ntuples}};
    \else
    \node (entuple\i) [myredbox1] at ($ (entuple_c) + (\i/2,\i/2) $)
    {};
    \fi
  ...
  % Geometry
  \node (gntuple) [myredbox1] at (gntuple_c) {\huge \textbf{Geometry}};

  % Geometry parquet
  \node (gparquet) [mybluebox2] at (gparquet_c) {\huge \twolines{Geometry}{\textit{.parquet}}};

  % Request
  \node (request) [mydiam] at (request_c) {\huge \textbf{Request}};
  ...
  % Reco Chain
  \node (chain) [mygreenbox1] at (chain_c) {\huge \textbf{Reco Chain}};

  % ...
  \foreach \i in {1,...,-1}{
    \ifnum\i=-1
    \node (future) [mygreenbox1] at ($ (future_c) + (\i/6.,\i/6.) $)
    {\huge \textbf{Future...}};
    \else
    \node (future) [mygreenbox1] at ($ (future_c) + (\i/6,\i/6) $)
    {};
  ...
  \draw [myarrow] (ecache.east) to [bend left=10] (chain.west);
  \draw [myarrow] (ecache.east) to [bend left=-20] (future.west);
  ...
  % Request arrows
  \draw [myarrow] (request.north) -- (eparquet.south);
  \draw [myarrow] (request.south) -- (gparquet.north);
  \draw [myarrowbend] ($ (request.west) + (.5,.5) $) arc (30:330:1cm);

  % Reconstruction chain arrows
  \draw [myarrow] (chain.east) to [bend left=50] (gdisplay2d.east);
  \draw [myarrow] (chain.east) to [bend left=50] (gdisplay3d.east);  
