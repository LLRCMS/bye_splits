...
branches = lambda tree, string: common_branches + [
    branch for branch in tree.keys() if string in branch
]
parquet_name = (
    lambda particle, string: f"{data_dir}PU200/{particle}/{string}_df.parquet"
)
    ...
    for key, val in input_files.items():
        if os.path.exists(val):
            with up.open(val) as rootFile:
                ...
                tc_branches, genpart_branches, cl3d_branches = (
                    branches(tree, "good_tc"),
                    branches(tree, "genpart"),
                    branches(tree, "cl3d"),
                )

                tc_outfile, genpart_outfile, cl3d_outfile = (
                    parquet_name(key, "tc"),
                    parquet_name(key, "genpart"),
                    parquet_name(key, "cl3d"),
                )
                ...
                if not os.path.exists(genpart_outfile):
                    ...
                    genpart_df = pd.merge(
                        genpart_df[0].drop("event", axis=1),
                        genpart_df[1],
                        on=["entry", "subentry"],
                    )
                    genpart_df = genpart_df.rename(
                        {
                            "good_genpart_exphi": "good_genpart_phi",
                            "good_genpart_exeta": "good_genpart_eta",
                        },
                        axis=1,
                    )
                    genpart_df["good_genpart_pt"] = genpart_df[
                        "good_genpart_energy"
                    ] / np.cosh(genpart_df["good_genpart_eta"])
...
def create_figures(phot_df, el_df, ptype, hist_bins):
    ...
    fig_enpt = make_subplots(
        rows=1, cols=2, subplot_titles=(r"$\Huge{p_T}$", r"$\Huge{Energy}$")
    )

    fig_enpt.update_layout(
        barmode="overlay",
        title_text=f"{ptype} Energy/PT Distributions",
        yaxis_title_text=r"$\Large{Events}$",
    )

    # Eta and Phi Distributions
    fig_ang = make_subplots(
        rows=1, cols=2, subplot_titles=(r"$\Huge{\eta}$", r"$\Huge{\phi}$")
    )

    fig_ang.update_layout(
        barmode="overlay",
        title_text=f"{ptype} Angular Distributions",
        yaxis_title_text=r"$\Large{Events}$",
    )

    for particle, color in zip(["Photons", "Electrons"], [phot_color, el_color]):
        for var in ["pt", "energy"]:
            fig_enpt.add_trace(
                go.Histogram(
                    x=phot_df[f"good_{ptype}_{var}"]
                    if particle == "Photons"
                    else el_df[f"good_{ptype}_{var}"],
                    nbinsx=hist_bins,
                    marker_color=color,
                    autobinx=False,
                    name=particle,
                    legendgroup=particle,
                    showlegend=True if var == "pt" else False,
                ),
                row=1,
                col=1 if var == "pt" else 2,
            )
        ...
        for var in ["eta", "phi"]:
            fig_ang.add_trace(
                go.Histogram(
                    ...
                    name=particle,
                    legendgroup=particle,
                    showlegend=True if var == "eta" else False,
                ),
                row=1,
                col=1 if var == "eta" else 2,
            )
...
def plot_dists(hist_bins, eta_range, particle_type, min_pt, min_en):
    ...
    phot_cl3d_df = phot_cl3d_df[
        (phot_cl3d_df.good_cl3d_eta > eta_range[0])
        & (phot_cl3d_df.good_cl3d_eta < eta_range[1])
    ]
    el_cl3d_df = el_cl3d_df[
        (el_cl3d_df.good_cl3d_eta > eta_range[0])
        & (el_cl3d_df.good_cl3d_eta < eta_range[1])
    ]

    # Apply energy and pt thresholds
    phot_cl3d_df = phot_cl3d_df[
        (phot_cl3d_df.good_cl3d_pt > min_pt) & (phot_cl3d_df.good_cl3d_energy > min_en)
    ]
    el_cl3d_df = el_cl3d_df[
        (el_cl3d_df.good_cl3d_pt > min_pt) & (el_cl3d_df.good_cl3d_energy > min_en)
    ]

    # Genpart DFs
    phot_genpart_df, el_genpart_df = (
        df_dict["photons"]["genpart"],
        df_dict["electrons"]["genpart"],
    )

    phot_genpart_df = phot_genpart_df[
        (phot_genpart_df.good_genpart_eta > eta_range[0])
        & (phot_genpart_df.good_genpart_eta < eta_range[1])
    ]
    el_genpart_df = el_genpart_df[
        (el_genpart_df.good_genpart_eta > eta_range[0])
        & (el_genpart_df.good_genpart_eta < eta_range[1])
    ]

    phot_genpart_df = phot_genpart_df[
        (phot_genpart_df.good_genpart_pt > min_pt)
        & (phot_genpart_df.good_genpart_energy > min_en)
    ]
    el_genpart_df = el_genpart_df[
        (el_genpart_df.good_genpart_pt > min_pt)
        & (el_genpart_df.good_genpart_energy > min_en)
    ]

    if particle_type == "Reco":
        fig_enpt, fig_ang = create_figures(phot_cl3d_df, el_cl3d_df, "cl3d", hist_bins)
    elif particle_type == "Genpart":
        fig_enpt, fig_ang = create_figures(
            phot_genpart_df, el_genpart_df, "genpart", hist_bins
        )
    else:
