...
def binned_effs(df, norm, perc=0.1):
    ...
    if not perc==1.:
        ...
        for i in range(1, 101):
            match_column = df.loc[df[norm].between(current_en, (i)*en_bin_size, 'left'), 'matches']
...
def display_color(coef, eta_range, normby, pileup):
    ...
    phot_df = (phot_df[ (phot_df.genpart_exeta > eta_range[0]) & (phot_df.genpart_exeta < eta_range[1]) ])
    pion_df = (pion_df[ (pion_df.genpart_exeta > eta_range[0]) & (pion_df.genpart_exeta < eta_range[1]) ])
...
def write_eff_file(norm, coefs, eta, file):    
    ...
    for coef in coefs[1:]:
        ...
        phot_df = (phot_df[ (phot_df.genpart_exeta > eta[0]) & (phot_df.genpart_exeta < eta[1]) ])
        pion_df = (pion_df[ (pion_df.genpart_exeta > eta[0]) & (pion_df.genpart_exeta < eta[1]) ])

        binned_var = 'genpart_energy' if norm == 'Energy' else 'genpart_pt'
        phot_eff, _ = binned_effs(phot_df, binned_var, 1.)
        pion_eff, _ = binned_effs(pion_df, binned_var, 1.)
...
def global_effs(eta_range, normby, pileup, file="global_eff.hdf5"):
    ...
    filename_user = "{}{}".format(f"{data_dir}PU0/", filename)
    filename_iehle = "{}{}{}/{}".format(cfgprod['dirs']['EhleDir'], cfgprod['dirs']['DataFolder'], pile_up_dir, filename)
