...
def process_trigger_cell_geometry_data(
    region, selection, positive_endcap_only=True, debug=False, **kw
):
    ...
    tcData, subdetCond = common.tc_base_selection(
        tcData,
        region=region,
        pos_endcap=True,
    ...
    with h5py.File(inoptfile, mode="w") as store:
        ...
        saveData_inv = (
            tcData_inv[save_cols].sort_values(by=["Rz_bin", "phi"]).to_numpy()
        )
        saveData_main = (
            tcData_main[save_cols].sort_values(by=["Rz_bin", "phi"]).to_numpy()
        )
...
def optimization(pars, **kw):
    ...
    dp = utils.data_processing.DataProcessing(
        phi_bounds=(kw["MinPhi"], kw["MaxPhi"]), bin_bounds=(0, 50)
    )

    # check detids make sense
    assert (
        np.sort(np.unique(store_in["data_inv"][:, -1]))
        == np.sort(store_in["data_inv"][:, -1])
    ).all()
    assert (
        np.sort(np.unique(store_in["data_main"][:, -1]))
        == np.sort(store_in["data_main"][:, -1])
    ).all()
    ...
    data_main, bins_main, _, _, idx_d_main = dp.preprocess(
        data=store_in["data_main"], **data_opt
    )
    data_inv, bins_inv, _, _, idx_d_inv = dp.preprocess(
        data=store_in["data_inv"], **data_opt
    )
    ...
    for rzslice, (ldata_main, lbins_main) in enumerate(zip(data_main, bins_main)):
        ...
        run_algorithm = (
            True  # this flag cna be used to apply the algo to a subset of the rzslices
        )
        ...
        if run_algorithm:
            ...
            phi_new_low_edges = kw["PhiBinEdges"][:-1][
                ld[:, idx_d_main.phibin].astype(int)
            ]
            phi_new_high_edges = kw["PhiBinEdges"][1:][
                ld[:, idx_d_main.phibin].astype(int)
            ]
            ...
            assert not np.count_nonzero(
                df.move_to_the_left * df.move_to_the_right != 0.0
            )

            # fix boundary conditions
            df.loc[
                df.bin_old - df.bin_new == kw["NbinsPhi"] - 1, "move_to_the_left"
            ] = 0
            df.loc[
                df.bin_old - df.bin_new == kw["NbinsPhi"] - 1, "move_to_the_right"
            ] = 1
            df.loc[
                df.bin_old - df.bin_new == -(kw["NbinsPhi"] - 1), "move_to_the_left"
            ] = 1
            ...
            df["d_left"] = df.move_to_the_left * abs(
                phi_old - phi_new_high_edges + half_bin_width
            )
            df["d_right"] = df.move_to_the_right * abs(
                phi_new_low_edges - phi_old + half_bin_width
            )
            assert not np.count_nonzero(df.d_left * df.d_right != 0.0)

            df["distance"] = -1 * df.d_left + df.d_right

            # the distance is zero when the bin does not change
            df.loc[df.bin_old == df.bin_new, "distance"] = 0.0

            nonzero_ratio = 1.0 - float(len(df[df.distance == 0])) / float(
                len(df.distance)
            )

            # remove migrations in boundary conditions to avoid visualization issues
            df.loc[df.distance >= np.pi, "distance"] = abs(
            ...
            plot_obj.save_phi_distances(
                phi_dist=df.distance, eucl_dist=eucl_dist, arc_dist=df.arc
            )
            plot_obj.save_iterative_phi_tab(
                nonzero_ratio=nonzero_ratio, ncellstot=ncellstot
            )
    ...
    for rzslice, (ldata_inv, lbins_inv) in enumerate(zip(data_inv, bins_inv)):
        ...
        df_inv = pd.DataFrame(
            dict(phi=phi_inv, id=np.uint32(ld_inv[:, idx_d_inv.tc_id]))
        )
        df_inv_total = (
            df_inv if rzslice == 0 else pd.concat((df_inv_total, df_inv), axis=0)
        )

        # end loop over the layers

    plot_name = common.fill_path("tc_moves", ext="html", **pars)
    plot_obj.plot_iterative(
        plot_name=plot_name,
...
if __name__ == "__main__":
    ...
    assert FLAGS.sel in ("splits_only", "no_splits") or FLAGS.sel.startswith(
        "above_eta_"
    )

    if FLAGS.process:
        process_trigger_cell_geometry_data(
            region=FLAGS.reg, selection=FLAGS.sel, **params.opt_kw
        )
    ...
    with open(outcsv, "w", newline="") as csvfile, pd.HDFStore(
        outresen, mode="w"
    ) as storeEnRes, pd.HDFStore(outrespos, mode="w") as storePosRes:
        fieldnames = [
            "ipar",
            "c_loc1",
            "c_loc2",
