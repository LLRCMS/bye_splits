...
where =-h/--help= can be used to inspect all options. You can adapt the above to your needs, exploiting the separate tasks defined under =bye_splits/tasks/=. A plotting step for validation is included.
